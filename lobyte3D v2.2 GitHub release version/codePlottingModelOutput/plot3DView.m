function plot3DView(glob, depos, newFigure, lobeOrFlowColourCoding, transparency, kmFlag)
    ...
    for y = 1:glob.ySize-1
       for x = 1:glob.xSize-1 
           ...
           if  deposThickness(y,x) > 0.01 % Draw a patch at xy for the flow that is at the top of the strata at xy
                yco = [y*glob.dy, y*glob.dy, (y+1)*glob.dy, (y+1)*glob.dy] .* xyScale;  
                xco = [x*glob.dx, (x+1)*glob.dx, (x+1)*glob.dx, x*glob.dx] .* xyScale;

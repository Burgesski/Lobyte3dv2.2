...
% Code also assumes that glob.dx and glob.dy the grid cell size variables are in meters
    ...
    % Check to see the plot limits are OK to plot. For dip sections where there is no deposition or deposition below prescribed thickness limit
    % plot limits might return unset. Also, function could be called with similarly unsuitable plot limits, so important to check to avoid crash on length of zxo and yco vectors
    if plotLimits.upDipLimit < plotLimits.downDipLimit 
        ...
        line([plotLimits.upDipLimit, plotLimits.downDipLimit] .* plotDy, [unitBaseMid, unitBaseMid], 'LineStyle','none');
        ...
        line([plotLimits.upDipLimit, plotLimits.downDipLimit] .* plotDy, [t,t], 'LineStyle','none');
        hold on;
